...
Only changed normalized fields are submitted to `PATCH /api/v1/lobbies/:id`. Background invalidation or manual GET cannot overwrite an unsaved draft. A synchronous save lock prevents double submission. A correctly shaped response with the expected id confirms saving, returns to updated details and invalidates all/mine/full mine/Search/inbox through the existing channel. A renamed event may no longer match the current search. Old reads and callbacks cannot replace newer details or another account/opening.
