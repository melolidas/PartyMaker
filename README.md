...
`GET /api/v1/notifications/unread-count` returns `{ unreadCount }`: the exact PostgreSQL count of the Bearer user's unread LOBBY_JOINED records across all pages, including null actor/lobby and cancelled-lobby notifications. Query parameters are rejected. Other notification types and read records are excluded; the aggregate never confirms any particular row's read state.
...
No push delivery, polling, WebSocket, mark-all, unread reversal, deletion, or edit/cancel/chat/other-type notifications. Other users' events appear at the documented refresh points, not realtime. Native/physical-phone testing is separate from the browser smoke test. Schema, seed and Auth/SessionCoordinator are unchanged.
