...
Profile name, bio, city, country code, extroversion level and avatar are backed by the API. Home's upcoming catalog, Your lobbies / View all, Search, lobby details, Create Lobby, the paper-plane chat inbox and lobby text chat use PostgreSQL through the same authenticated ApiClient. Profile → Lobbies now shows real scheduled participation history. Gallery and statistics remain explicitly labelled demos only under Profile → Moments; Activity shows real LOBBY_JOINED and LOBBY_CANCELLED notifications. Real joining/leaving and organizer cancellation work, including from search results. Basic organizer editing is available for future lobbies. Invitations, push/other notification types, organizer transfer, schedule editing, physical deletion and media other than profile avatars are not implemented.
...
The avatar is **public**: anyone with its URL can download it without signing in. The URL comes from the existing configured API base, never a hardcoded host. Replaced/unassigned files are not served, but replacing or removing the assignment cannot revoke already-downloaded public copies or an in-flight download. **Remove profile photo / Убрать фото профиля** detaches the assignment only, without deleting the account, MediaAsset or disk file.

Removal uses `DELETE /api/v1/users/me/avatar/:avatarId` with Bearer, a UUID and no body/query. The same User row lock as replacement serializes both actions: matching id → null; already null → successful no-op without updatedAt changes; another current id → `409 AVATAR_CHANGED`. An old retry of removal A cannot remove a newly assigned B. Unassigned public GET returns `404 AVATAR_NOT_FOUND`.

The existing editor holds a frozen target per explicit RU/EN confirmation. Declining does not send DELETE. One synchronous busy gate covers picking, upload, reload and removal; a draft selection is separate and survives removal. Only an exact `{avatar:null}` receipt shows removal success and updates Profile/editor/nav to neutral placeholders. Network/5xx/invalid receipts retain the original target for manual retry; Reload updates only the current avatar, not the earlier DELETE's confirmation. AVATAR_CHANGED asks for a profile reload; deleting a new target requires a new confirmation. No automatic network retry (the existing single auth-rejection refresh retry remains). Field ownership keeps text/bio/extroversion independent; close/account/logout/recovery invalidate pending editor operations and late callbacks. No durable storage for pending removal, schema changes or cleanup jobs.

Avatar removal verification: frontend typecheck and all 359 tests; backend typecheck/lint/build and all 135 tests against the existing PartyMaker PostgreSQL on localhost:55432; then `test:lobby-edit:integration` passed. Expo Web smoke used one new isolated account: file picker/PNG upload → decline removal (assignment/updatedAt unchanged) → confirm removal with a separate local draft → placeholders on Profile/editor/BottomNav and distinct removal receipt → sign out/in still has no avatar → select/upload a new PNG successfully. The detached public URL returned 404 while its MediaAsset and processed file remained intact. Only this smoke account, its two media records/files and generated source fixtures were cleaned up afterward. Conditional-conflict, lost-response/retry, concurrency, RU/EN and late-callback cases are automated tests, not browser fault-injection checks. A physical phone was not tested.
...
- Activity: real organizer join / participant cancellation notifications, exact unread badge, individual read action, manual refresh
...
Browser regression: create an isolated lobby → Home personal section → View all → details → logout/relogin → verify membership persists. Sign in as a different isolated user: the lobby may appear in all, but not mine without their own JOINED membership. Also test the empty/Create state and Refresh → error → Retry with isolated request failure, without interrupting shared services. Use known fixture ids; never reset/reseed or modify pre-existing records. Home remains future-only; the separate Profile history described below does not alter scope=mine. Physical deletion and lobby/message media remain unavailable.
...
The nearest 200 candidates are filtered **in PostgreSQL before LIMIT**: future PUBLISHED, free places counting all JOINED, not organized by you, and no membership for you of any status (including LEFT/REMOVED). Ordinary catalog rejoining is unchanged. One server time and RepeatableRead snapshot cover sources, candidates and the safe final Lobby projection.
...
Use the central + button. Enter title, description, date (`YYYY-MM-DD`), 24-hour time (`HH:MM`), capacity and meeting format. There is no category selector or hidden category default: the form omits `category`, and the API stores/returns `category: null`. Uncategorized lobbies use a neutral people icon without a category label throughout Home, Search, details, chats and Profile history. Existing categorized lobbies retain their values and labels; no old data is rewritten. In-person events require a venue name; online events send `venueName: null`. The form explicitly uses **Asia/Bishkek (UTC+06:00)**, not the device/computer timezone. It checks the calendar and round-trips the named timezone, rejecting impossible dates such as February 30 rather than normalizing them. Photos, maps and geocoding are visibly unavailable; no demo photo is preselected.

Existing installations: from `backend/`, run `npm run prisma:migrate:deploy` and `npm run prisma:generate`, then rebuild/restart the API. Migration `20260906200000_optional_lobby_category` only drops the NOT NULL constraint on `Lobby.category`. It does not reset/reseed, delete data, assign replacement categories or add recommendations. For compatibility the API still accepts a valid legacy category on creation, or explicit null; other non-null values remain invalid.
...
The creation form can include up to **20 optional roles** after its basic fields. Add role is the first 48×48 outlined circle; the shared white SVG half-mask represents every role. Tiles wrap onto complete rows. A popup adds a trimmed name (1–10 Unicode code points) and optional description (0–50); counters use the same code-point limits as the API. Existing draft roles are read-only with a Remove action. Closing without adding makes no changes. Until lobby submission, all roles remain local. Creation without roles/category retains the previous behavior.

Lobby, ordered role definitions and organizer membership commit together. The additive `20260907200000_lobby_activity_roles` migration creates `LobbyActivityRole` and `LobbyRoleAssignment`; existing lobbies acquire no roles. Apply migrations and generate Prisma from `backend/` before rebuilding/restarting the API, using the existing database, without reset or seed.

In a JOINED participant's real chat, **Choose role / Выбрать роль** appears only when roles exist. The grid and role details replace overlay content inside the current native Modal. Back/Escape goes role details → grid → chat. A free role can be selected, an occupied role is view-only, and one's own role can be released. Choosing another free role atomically replaces the previous one; an occupied target leaves it unchanged. The organizer follows the same rules. These are activities/duties, **not ORGANIZER/MEMBER permissions**, global-profile roles or categories. No role is assigned automatically.

One participant per role and one role per participant/lobby are enforced by database constraints. Selection, specific-role release and leave share the existing Lobby row lock. A late release of A cannot remove B. Leave frees the role atomically; rejoin does not restore it. Access remains JOINED-only in PUBLISHED lobbies, including already-started events, never CANCELLED/COMPLETED. Names beside message authors and roster members show the **current** assignment, not the role at message creation; messages remain unchanged. A single bounded role lookup supplies all badges, without loading every participant or per-message requests.

Assignment is shown only after a validated server receipt. Uncertain network/server/response results keep the exact role/action for an explicit retry; no automatic network retries, apart from the existing bounded auth-refresh retry after a definite 401. Role errors do not reset message drafts or pending sends. Old responses after account/lobby change, closure or invalidation are ignored. Other participants' changes appear on existing Refresh/reopening, not polling/realtime. Published role definitions cannot yet be edited/deleted; there is no mandatory assignment, automatic distribution, extra imagery or push.

Role endpoints and errors are documented in backend README and Swagger. Browser verification at 320/390 px covers the mask, wrapped draft grid, add/view/remove/cancel, creation, two-account occupied/free roles, selection/switch/release, unchanged messages with current badges, leave/rejoin, persisted assignment after relogin and both details/inbox Back/Escape paths. Only one confirmed fixture lobby and its two fixture users/related rows were removed. Database concurrency/error races and late-response behavior are automated tests, not browser failure injection. No physical phone was tested.
...
## Profile → Lobbies: scheduled participation history

Profile's tabs actually switch content while keeping its header, avatar and editors mounted. **Moments** retains the demo gallery and its explicitly labelled 24/18/128 statistics. **Lobbies** loads only `GET /api/v1/users/me/lobby-history?limit=20&after=…` through the existing authenticated ApiClient; there are no counts or demo fallbacks in history.

History includes own JOINED memberships in PUBLISHED or COMPLETED lobbies whose `startsAt <= serverNow`. One server time is captured for each request. Future, CANCELLED, DRAFT, LEFT, REMOVED and non-member lobbies are excluded, even for their organizer. This is recorded participation by the scheduled **start**, not proof of attendance or that a meeting has ended. No endsAt, automatic status changes, background jobs or physical movement of records are introduced.

Cards contain only real title/description/category, date/time in the original IANA time zone, online/venue and canonical Organizer/Participant role. They are informational: no join/leave/edit/cancel, details, roster or chat navigation. COMPLETED history does not grant access to those existing endpoints. No avatar/profile/editor data is updated by history responses.

The server filters before `limit+1`, ordering `startsAt DESC, id DESC` with a strict tuple cursor. Separate pages are not a frozen archive; Refresh or reopening discovers new leading entries. The tab owns loading/empty/error/Retry, Refresh, Load more and page errors preserving previous rows/cursor. IDs deduplicate; Refresh resets pagination. Account, tab close/reopen, logout/recovery, unmount and shared lobby invalidation discard stale reads. One existing vertical Profile scroller is used, without nested lists. No polling, new provider or totalCount.

History verification: frontend typecheck/366 tests; backend typecheck/lint/build/141 tests on the unchanged PartyMaker PG17 cluster at localhost:55432; then `test:lobby-edit:integration` passed. Expo Web smoke with two isolated accounts confirmed own past PUBLISHED/COMPLETED entries, original America/New_York dates, future events remaining on Home, exclusion of cancelled/LEFT entries, account isolation, tab switching, Refresh and persisted history after relogin. Only five fixture lobbies and their two users/related rows were removed. Exact time boundary, pagination/error races, RU/EN and editor-state preservation are automated tests, not browser failure injection. Physical-phone testing was not performed. Schema/migrations/seed/dependencies/auth protocol and Home/chat semantics are unchanged.
...
The organizer can open **Edit** from real details, inside the same Modal. Back/system Back discards the local draft without PATCH and returns to the same Home/Search/personal-list context. The editor reads current API data once on opening. It allows title, description, capacity and the linked online/venue pair; legacy categorized lobbies also retain their category controls. Uncategorized lobbies have no category controls and can be edited without assigning a category. PATCH still accepts only a valid enum when category is explicitly supplied, not null; leaving it out preserves the current value, including null. The original schedule and time zone are displayed read-only, never converted through the Bishkek create form and never included in PATCH.
...
Capacity cannot fall below JOINED count or persisted minParticipants. The organizer counts as joined; LEFT/REMOVED do not. Database writes share Lobby FOR UPDATE with membership/cancel/send, so independent changed fields merge; for the same field the last successful commit wins. Schedule changes, transfer, physical deletion, photos and edit notifications are not included. Other users see edits after Refresh/reopening, not realtime. Physical-phone behavior requires a separate device check.

## Activity: join and cancellation notifications

Activity loads the authenticated user's real `LOBBY_JOINED` and `LOBBY_CANCELLED` notifications on tab opening and manual Refresh. New absent/LEFT → JOINED participation creates an event for Lobby.organizerId in the SAME PostgreSQL transaction/parent row lock as membership. Failed notification insertion rolls back the join. No-op/concurrent duplicate joins, leave, rejected joins and organizer self-membership do not create events; a later real rejoin creates a new event. There is no backfill or change to seed data.

First PUBLISHED → CANCELLED creates one event for each current JOINED participant except the canonical organizer, atomically with the cancellation under the same Lobby FOR UPDATE lock used by join/leave/edit/send. LEFT/REMOVED/outsiders do not receive one. Insertion failure rolls back status and every notification. Repeated/concurrent cancel (including a lost-response retry after startsAt) is a no-op with no duplicate events. The organizer's badge is not increased by these participant notifications. Membership/message history is preserved.

`GET /api/v1/notifications?limit=20&after=…` filters recipient and the shared two-type allowlist before bounded `createdAt DESC, id DESC` pagination, with a consistent snapshot per page. Rows expose id/type/createdAt/readAt/lobbyTitleSnapshot, safe actor `{ id, displayName, handle, avatar }` or null, and lobby `{ id, title }` or null. Actor names/avatars are **current**. JOINED has a null snapshot and exposes only a current PUBLISHED lobby/title. CANCELLED has the title saved under the lock at cancellation (or null with a fallback), always lobby:null and no open-lobby button. This private historical title grants no access to cancelled details/chat. Cancelled/non-PUBLISHED lobbies are hidden without deleting the notification; deleted actors use a neutral placeholder. A notification records a past action, not current membership. Existing other notification types remain in the DB but are excluded from list/read/count.

Existing installations: from `backend/`, apply the reviewed additive migration with `npm run prisma:migrate:deploy`, then `npm run prisma:generate` before building/restarting the API. `20260906160000_lobby_cancelled_notifications` adds enum LOBBY_CANCELLED and nullable `Notification.lobby_title_snapshot VARCHAR(40)`; old records retain null. No backfill, reset/reseed, table recreation or connection change. See the backend README for the local cluster.
...
`GET /api/v1/notifications/unread-count` returns `{ unreadCount }`: the exact PostgreSQL count of the Bearer user's unread LOBBY_JOINED and LOBBY_CANCELLED records across all pages, including null actor/lobby regardless of current membership/lobby status. Query parameters are rejected. Other notification types and read records are excluded; the aggregate never confirms any particular row's read state.

The Activity button shows no numeric badge for confirmed 0, 1–99 literally and 99+ above that. `…` means updating, `?` means unavailable (open Activity and Refresh to retry), never a fabricated zero or a stale number presented as fresh. RU/EN accessibility labels expose the full confirmed count. The non-interactive badge stays inside the existing icon area in normal/compact navigation.

One authenticated feature provider owns count state above tabs. It fetches after login/restore, Activity opening/manual Refresh, a confirmed mark-read POST (even after closing Activity), and a GET/page readAt resolving earlier uncertainty. Requests coalesce; an invalidation during a request discards its result/error and schedules a trailing read. There is no local decrement or automatic error retry. Account/logout/recovery/unmount boundaries hide and discard old state/responses. Closing Activity does not dispose the counter, and count responses do not touch its rows or cancellation receipts.

Verified in Expo Web with two isolated accounts: create/join → Activity count 1 → mark read → no numeric badge → sign out/in preserves 0; the member's counter/list remain separate. A longer fixture history verified pagination, count 21 and the badge in normal/compact navigation. Only the fixture lobby, its events/memberships and the two fixture users were removed. Physical-phone testing was not performed; 99+ and error/race cases are covered by automated component/store tests.

Cancellation slice verified in Expo Web with two new isolated accounts: organizer creates → member joins → organizer cancels from Activity → member opens/refreshes Activity and sees the saved title, count 1 and no lobby link → marks read → count 0 and read state persist after sign-out/in. The organizer retains the confirmed cancellation notice and does not receive a self-event or a badge increase. PostgreSQL verification confirmed both memberships were preserved and each recipient had only its own event before exact fixture cleanup. Automated checks: frontend 351 tests/typecheck; backend 131 tests/typecheck/lint/build; then the real PostgreSQL lost-PATCH-response integration test. Mixed pagination, null fallbacks, RU/EN, races and rollback are automated; physical phone was not tested.

No push delivery, polling, WebSocket, mark-all, unread reversal, deletion, or edit/chat/other-type notifications. Recipients see cancellations at the existing refresh points (login, Activity opening and Refresh), not instantly on another device. Native/physical-phone testing is separate from the browser smoke test. Only the additive notification schema change above is needed; seed, dependencies and Auth/SessionCoordinator are unchanged.
