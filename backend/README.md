...
The ReadCommitted transaction takes the same parameterized Lobby `FOR UPDATE` row lock as join/leave/edit/send, then rechecks existence, status, owner and time. First cancellation updates status/updatedAt and atomically inserts LOBBY_CANCELLED notifications for JOINED participants except Lobby.organizerId, capturing the current title under that lock. An insertion error rolls back the entire cancellation. The already-CANCELLED branch returns before inserts; replay/concurrent cancel cannot duplicate events. Membership rows/statuses/timestamps, messages and related records are neither deleted nor rewritten. A preceding join/leave/edit/send retains its committed effect; if cancellation commits first, those later actions return LOBBY_NOT_FOUND. No in-memory lock or background job. See the additive notification migration below.
...
There is no global DB/filesystem atomicity. Disk/processing failure preserves the old assignment. A failed/uncertain DB transaction deliberately does **not** delete the prepared file: it may already have been committed. Old MediaAsset rows/files are retained because other relations may still use them. Crash-orphan/old-file cleanup, storage quotas, physical media deletion, distributed storage, cloud/CDN and background jobs are deferred. Back up PostgreSQL and processed files together; do not purge the directory or all unassigned assets indiscriminately. `uploads/` and test artifacts are Git-ignored. This is a single-instance local MVP, not a general media platform.

`DELETE /api/v1/users/me/avatar/:avatarId` requires Bearer and a UUID, with no body (including empty JSON objects) or query parameters. Under the same User FOR UPDATE as replacement, it rereads the Bearer user's avatarMediaId: matching UUID → update only avatarMediaId=null; already null → 200 no-op without any UPDATE; another assigned UUID → 409 AVATAR_CHANGED. Success is exactly 200 `{ avatar: null }`. The id is a condition on one's own profile, not an asset deletion capability: foreign and arbitrary ids reveal no ownership/existence information and cannot affect another user. Replaying removal A after replacing with B preserves B. All other user fields and MediaAsset/files remain intact; no Prisma migration or seed changes. Public GET for a detached avatar returns AVATAR_NOT_FOUND, but downloaded copies and already-started reads cannot be recalled. Reloading a profile is not a receipt for a lost DELETE response; retry the same target explicitly or confirm a new one after AVATAR_CHANGED. CORS adds only DELETE to the existing methods; origins/credentials/auth policy are unchanged.
...
## Notifications: join and cancellation Activity slices

One shared allowlist supports `NotificationType.LOBBY_JOINED` and `LOBBY_CANCELLED` in list, read (including the UPDATE predicate), and unread-count. `GET /api/v1/notifications?limit=20&after=<cursor>` requires Bearer, filters recipientId from that user and supported types BEFORE pagination, and returns `{ items, nextCursor }`. Limit is 1–50; order/cursor are createdAt DESC, id DESC, mixed across both types. Unknown query fields, arrays/objects, unsupported dates/UUIDs and malformed cursors return 400 VALIDATION_FAILED. A RepeatableRead transaction provides a consistent bounded page and current related projections.

Each item: `{ id, type, createdAt, readAt, actor, lobby, lobbyTitleSnapshot }`. Actor is null if deleted; otherwise only current id/displayName/handle and the existing safe processed-avatar mapper. JOINED has lobbyTitleSnapshot:null and lobby id/current title only while PUBLISHED, otherwise null. CANCELLED always has lobby:null and exposes only its saved title-at-cancellation (nullable fallback), not details/chat access. No email/recipientId/auth/storage paths/Moment/Comment relations are serialized. Names and JOINED titles are current related values; cancellation title is a historical snapshot. Events do not guarantee current JOINED membership. Other seed notification types remain stored but are excluded from list/read/count; no backfill/reseed.

Migration `20260906160000_lobby_cancelled_notifications` is additive: `ALTER TYPE ... ADD VALUE 'LOBBY_CANCELLED'` and nullable `lobby_title_snapshot VARCHAR(40)` with the snake_case Prisma mapping. Existing records retain null; no tables or records are recreated/updated. Review the SQL, then from `backend/` run `npm run prisma:validate`, `npm run prisma:migrate:deploy`, `npm run prisma:generate` before building/restarting the updated backend. Do not accept reset or change DATABASE_URL. Applied to the original local PartyMaker PG17 cluster on port 55432; pre/post fingerprints of existing users, sessions, media, lobbies/members/messages/invites, moments and notifications matched (excluding the new null column).
...
`GET /api/v1/notifications/unread-count` requires Bearer, accepts no query parameters and returns only `{ unreadCount: nonnegative integer }`. It uses a database COUNT with recipientId=current user, type IN (LOBBY_JOINED, LOBBY_CANCELLED) and readAt=null, not list pagination or loaded rows. Null actor/lobby and any current membership/lobby status still count; other types and read records do not. Reading the same event repeatedly does not decrement twice. Existing indexes/seed remain unchanged. Swagger and PostgreSQL tests cover isolation, mixed types across pages, null relations, cancellation and idempotent/concurrent read.
...
Health, Auth, Profile including public avatars and conditional avatar detachment, Lobby creation/cancellation, catalog/details, join/leave, per-lobby text messages and the available-chat inbox are implemented. Basic organizer editing, LOBBY_JOINED/LOBBY_CANCELLED Activity/read and their exact unread count are also implemented. Other Lobby mutations, moments, other notification types and media other than profile avatars remain outside this stage; demo frontend records are kept separate from real lobbies.

Conditional avatar removal was verified by the full backend suite (135 tests), typecheck, lint and build on the existing local PostgreSQL cluster; root frontend checks (359 tests/typecheck) and the subsequent real lost-PATCH-response integration test passed. Tests include both overlapping replace/remove row-lock orders, unchanged no-op timestamps, retained assets/files, old public URL 404, identity isolation and DELETE preflight. Expo Web verified upload, declining/confirming removal, retained draft, three placeholders, persisted absence after relogin and a new upload. Only isolated fixture IDs/files were cleaned up. Physical-phone behavior and browser-level network-loss injection were not manually checked; failure/retry and session races are covered automatically. Schema, migrations, seed, dependencies and auth protocol were not changed by this slice.
