...
export class NotificationsService {
  ...
  async list(userId: string, query: ListNotificationsQueryDto): Promise<NotificationPageDto> {
    ...
    return this.prisma.$transaction(async tx => {
      ...
      const rows = await tx.notification.findMany({ where: { recipientId: userId, type: 'LOBBY_JOINED',
      ...
      return { items: page.map(row => ({ id: row.id, type: 'LOBBY_JOINED', createdAt: row.createdAt.toISOString(), readAt: row.readAt?.toISOString() ?? null,
        ...
        lobby: row.lobby?.status === 'PUBLISHED' ? { id: row.lobby.id, title: row.lobby.title } : null,
  ...
  async read(userId: string, id: string): Promise<NotificationReadDto> {
    return this.prisma.$transaction(async tx => {
      ...
      await tx.notification.updateMany({ where: { id, recipientId: userId, type: 'LOBBY_JOINED', readAt: null }, data: { readAt: new Date() } });
      const row = await tx.notification.findFirst({ where: { id, recipientId: userId, type: 'LOBBY_JOINED' }, select: { id: true, readAt: true } });
