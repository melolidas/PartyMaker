...
export function LiveChatsModal({ onClose }: { onClose: () => void }) {
  ...
  const [roleBack, setRoleBack] = useState<(() => void) | null>(null);
  const registerRoleBack = useCallback((back: (() => void) | null) => setRoleBack(() => back), []);
  ...
  useEffect(() => {
    store.setAccount(account); setSelected(null); setRoleBack(null);
  ...
  const returnToList = useCallback(() => { setRoleBack(null); setSelected(null); void store.reload(); }, [store]);
  return <Modal transparent animationType="none" presentationStyle="overFullScreen" onRequestClose={() => roleBack ? roleBack() : requestBack.current()} accessibilityLabel={selection?.lobby.title ?? t('chats.title')}>
    <GestureHandlerRootView style={styles.overlay}>
      <NavScrollContext.Provider value={ignoreNavScroll}>
        ...
        {selection ? <SwipeBackPage key={`${account}/${selection.lobby.id}`} name="conversation" edgeOnly onClose={returnToList} onBackReady={registerBack} backOverride={roleBack ?? undefined}>
          {(close, gesture) => <Screen scroll={false}><View style={styles.conversation}>
            <LiveLobbyChatScreen lobbyId={selection.lobby.id} title={selection.lobby.title} onBack={close} backLabel={t('liveChat.backToChats')}
              scrollGesture={gesture} onPopupBack={registerRoleBack} onSent={() => void store.reload()} onAccessLost={() => store.accessLost(selection.lobby.id)} />
          </View></Screen>}
        </SwipeBackPage> : null}
