...
export function LiveLobbyChatScreen({ lobbyId, title, onBack, onAccessLost, onSent, backLabel, scrollGesture }: {
  ...
  const sent = useRef(onSent); sent.current = onSent;
  const store = useMemo(() => new LiveLobbyChatStore(lobbyApi, () => uuid.v4(), () => lost.current(), () => sent.current?.()), [lobbyApi]);
  ...
  scroll.update(messages.map(message => message.id), current.confirmedSends);
  const scrollTo = (offset: number | null) => { if (offset !== null) list.current?.scrollToOffset({ offset, animated: false }); };
  ...
  const history = <FlatList ref={list} testID="live-chat-history" inverted style={styles.history} contentContainerStyle={styles.messages} data={messages} keyExtractor={item => item.id}
      keyboardShouldPersistTaps="handled" keyboardDismissMode={Platform.OS === 'ios' ? 'interactive' : 'on-drag'}
      onScroll={({ nativeEvent }) => scroll.onScroll(nativeEvent.contentOffset.y)} onScrollBeginDrag={scroll.onDrag} scrollEventThrottle={16}
      onContentSizeChange={(_width, height) => scrollTo(scroll.onSize(height))} onLayout={() => scrollTo(scroll.onViewport())}
      ListFooterComponent={<View style={styles.older}>
        {current.nextCursor && !current.blocked ? <Pressable testID="live-chat-older" accessibilityRole="button"
